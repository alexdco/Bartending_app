...
3. [x] Write the one-time `translate-catalog.ts` backfill script (mirrors spec 0011's `tag-regions.ts` shape) calling Claude Haiku per recipe (name, instructions, glass, ingredient measures) and per ingredient/tag into Spanish, computing and storing `source_name_hash`, upserting the new translation tables; its translation logic now lives in `packages/shared/src/catalogTranslation.ts` (exported via the `@bartendingapp/shared/catalog-translation` subpath, kept out of the main export barrel since it pulls in `@anthropic-ai/sdk`, a service role only dependency), shared with the import job's step 6 translation step, satisfies **AC-11**, **AC-15**. **Not yet run against the live catalog**: 0 rows exist in `recipe_translations`/`ingredient_translations`/`tag_translations` for the 427 recipes/295 ingredients/62 tags currently live; the engineer will run `pnpm --filter @bartendingapp/shared translate-catalog` themselves once `ANTHROPIC_API_KEY` is configured locally.
...
6. [x] Add the translation step to the existing import job (`packages/import-job/src/index.ts` and `importCustomRecipes.ts`, both entry points), calling the shared `translateCatalog` after their `import_catalog`/`import_custom_recipes` RPC call has already returned successfully, so it runs after the catalog transaction commits and outside its advisory lock; comparing each recipe's `source_name_hash` before calling Claude Haiku so only new or genuinely changed recipes are (re)translated; a missing `ANTHROPIC_API_KEY` or a translation failure logs a warning/error and returns without throwing, never failing the import itself, satisfies **AC-9**, **AC-17**.
7. [x] `locale` on `Profile`/`fetchProfile` and `updateLocale` alongside `updateDisplayName` in `packages/shared/src/preferences.ts` were already in place from step 6; added `useUpdateLocale` hooks (web `apps/web/src/auth/use-preferences.ts`, mobile `apps/mobile/src/auth/use-preferences.ts`) mirroring `useUpdateDisplayName`. Wired the locale switch control: web gets an always visible `LanguageMenu`/`DrawerLanguageSection` in `site-nav.tsx` (a route change via next-intl's router, plus an account sync when signed in); mobile gets a `LanguageCard` on the account screen (`apps/mobile/src/app/account.tsx`, persisted via `changeLocale`/AsyncStorage, plus the same account sync). On sign in, `signInWithPassword` (`packages/shared/src/auth.ts`) now takes an optional `guestLocale` and writes it to `user_preferences.locale` only when the account has none yet (`fetchProfile().locale === null`), satisfying **AC-16**; both web's and mobile's `useSignIn` pass the caller's active locale. UI string translation was scoped down for this pass (agreed with the engineer): the nav (already done pre-step-7), the web/mobile account screens, and the web privacy page are now fully translated (`apps/web/messages/{en,es}.json`, `apps/mobile/src/i18n/locales/{en,es}.json`), satisfying **AC-4** and the account/privacy slice of **AC-1**. **Not done**: the rest of **AC-1** — search, pantry, favorites, drink ideas, popular, and recipe detail pages/screens on both apps still have hardcoded English strings; this is a large mechanical sweep tracked as follow up, not folded into step 7.
