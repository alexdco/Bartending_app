...
- [ ] Build it: `/develop multi language support`
  ...
  - [ ] Locale switch control, sign in merge behavior, and `generate-drink-idea`'s locale aware generation with corrected pantry verification, satisfies AC-4, AC-10, AC-16. Locale switch control (web nav + mobile account screen) and sign in merge behavior are done, satisfying AC-4 and AC-16; `generate-drink-idea`'s locale aware generation and pantry verification fix (AC-10) is still pending, tracked in spec 0020 build plan step 8.
