-- Two distinct source ingredient/tag names (e.g. differing only in case or
-- whitespace) can normalize to the same canonical row within one recipe.
-- Guard both join inserts so a within-recipe collision is a no-op, not a
-- unique violation on (recipe_id, ingredient_id) / (recipe_id, tag_id).
